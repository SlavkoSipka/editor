...
# Run R2 sync + library indexing in the background so /health is reachable
# immediately and Railway's healthcheck doesn't time out while we embed.
bootstrap() {
  ...
  if [ -n "${R2_BUCKET:-}" ] && [ -n "${R2_ENDPOINT_URL:-}" ] && \
    ...
    if [ ! -f "$DATA_DIR/library/manifest.json" ] || [ "${R2_SYNC_FORCE:-}" = "1" ]; then
      ...
      python -m src.utils.r2_sync --prefix library --dest "$DATA_DIR/library" || echo "WARNING: R2 library sync failed"
      python -m src.utils.r2_sync --prefix embeddings --dest "$DATA_DIR/embeddings" || echo "WARNING: R2 embeddings sync failed"
    else
